...
int usage(int ret) {
  if ((! GetConsoleWindow() || ! GetStdHandle(STD_OUTPUT_HANDLE)) && GetProcessWindowStation()) popup_message(0, MB_OK, CWSM_MESSAGE_USAGE, CWSM_VERSION, CWSM_CONFIGURATION, CWSM_DATE);
  else print_message(stderr, CWSM_MESSAGE_USAGE, CWSM_VERSION, CWSM_CONFIGURATION, CWSM_DATE);
  return(ret);
}
...
static int elevate(int argc, TCHAR **argv, unsigned long message) {
  ...
  sei.lpVerb = _T("runas");
  sei.lpFile = (TCHAR *) cwsm_imagepath();

  TCHAR *args = (TCHAR *) HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, EXE_LENGTH * sizeof(TCHAR));
  if (! args) {
    print_message(stderr, CWSM_MESSAGE_OUT_OF_MEMORY, _T("GetCommandLine()"), _T("elevate()"));
    return 111;
  }

  /* Get command line, which includes the path to CWSM, and skip that part. */
...
const TCHAR *cwsm_unquoted_imagepath() {
  return unquoted_imagepath;
}

const TCHAR *cwsm_imagepath() {
  return imagepath;
}

const TCHAR *cwsm_exe() {
  return imageargv0;
}
...
int _tmain(int argc, TCHAR **argv) {
  ...
  if (argc > 1) {
    ...
    */
    if (is_version(argv[1])) {
      _tprintf(_T("%s %s %s %s\n"), CWSM, CWSM_VERSION, CWSM_CONFIGURATION, CWSM_DATE);
      cwsm_exit(0);
    }
    if (str_equiv(argv[1], _T("start"))) cwsm_exit(control_service(CWSM_SERVICE_CONTROL_START, argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("stop"))) cwsm_exit(control_service(SERVICE_CONTROL_STOP, argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("restart"))) {
      int ret = control_service(SERVICE_CONTROL_STOP, argc - 2, argv + 2);
      if (ret) cwsm_exit(ret);
      cwsm_exit(control_service(CWSM_SERVICE_CONTROL_START, argc - 2, argv + 2));
    }
    if (str_equiv(argv[1], _T("pause"))) cwsm_exit(control_service(SERVICE_CONTROL_PAUSE, argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("continue"))) cwsm_exit(control_service(SERVICE_CONTROL_CONTINUE, argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("status"))) cwsm_exit(control_service(SERVICE_CONTROL_INTERROGATE, argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("statuscode"))) cwsm_exit(control_service(SERVICE_CONTROL_INTERROGATE, argc - 2, argv + 2, true));
    if (str_equiv(argv[1], _T("rotate"))) cwsm_exit(control_service(CWSM_SERVICE_CONTROL_ROTATE, argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("install"))) {
      if (! is_admin) cwsm_exit(elevate(argc, argv, CWSM_MESSAGE_NOT_ADMINISTRATOR_CANNOT_INSTALL));
      create_messages();
      cwsm_exit(pre_install_service(argc - 2, argv + 2));
    }
    if (str_equiv(argv[1], _T("edit")) || str_equiv(argv[1], _T("get")) || str_equiv(argv[1], _T("set")) || str_equiv(argv[1], _T("reset")) || str_equiv(argv[1], _T("unset")) || str_equiv(argv[1], _T("dump"))) {
      ...
      if (ret == 3 && ! is_admin && argc == 3) cwsm_exit(elevate(argc, argv, CWSM_MESSAGE_NOT_ADMINISTRATOR_CANNOT_EDIT));
      /* There might be a password here. */
      for (int i = 0; i < argc; i++) SecureZeroMemory(argv[i], _tcslen(argv[i]) * sizeof(TCHAR));
      cwsm_exit(ret);
    }
    if (str_equiv(argv[1], _T("list"))) cwsm_exit(list_cwsm_services(argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("processes"))) cwsm_exit(service_process_tree(argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("remove"))) {
      if (! is_admin) cwsm_exit(elevate(argc, argv, CWSM_MESSAGE_NOT_ADMINISTRATOR_CANNOT_REMOVE));
      cwsm_exit(pre_remove_service(argc - 2, argv + 2));
    }
  }
  ...
  */
  if (! GetStdHandle(STD_INPUT_HANDLE)) {
    /* Start service magic */
    SERVICE_TABLE_ENTRY table[] = { { CWSM, service_main }, { 0, 0 } };
    if (! StartServiceCtrlDispatcher(table)) {
      unsigned long error = GetLastError();
      /* User probably ran cwsm with no argument */
      if (error == ERROR_FAILED_SERVICE_CONTROLLER_CONNECT) cwsm_exit(usage(1));
      log_event(EVENTLOG_ERROR_TYPE, CWSM_EVENT_DISPATCHER_FAILED, error_string(error), 0);
      cwsm_exit(100);
    }
  }
  else cwsm_exit(usage(1));
