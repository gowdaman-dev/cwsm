...
CWSM is a service helper program similar to srvany and cygrunsrv.  It can 
start any application as an NT service and will restart the service if it 
fails for any reason.

CWSM also has a graphical service installer and remover.

Full documentation can be found online at

                              https://github.com/Sree-Cognicoders/Chronexa-ws-manager

Since version 2.0, the GUI can be bypassed by entering all appropriate 
options on the command line.

Since version 2.1, CWSM can be compiled for x64 platforms.
Thanks Benjamin Mayrargue.

Since version 2.2, CWSM can be configured to take different actions
based on the exit code of the managed application.

Since version 2.3, CWSM logs to the Windows event log more elegantly.

Since version 2.5, CWSM respects environment variables in its parameters.

Since version 2.8, CWSM tries harder to shut down the managed application
gracefully and throttles restart attempts if the application doesn't run
for a minimum amount of time.

Since version 2.11, CWSM respects srvany's AppEnvironment parameter.

Since version 2.13, CWSM is translated into French.
Thanks François-Régis Tardy.

Since version 2.15, CWSM is translated into Italian.
Thanks Riccardo Gusmeroli.

Since version 2.17, CWSM can try to shut down console applications by
...
Since version 2.17, CWSM can redirect the managed application's I/O streams
to an arbitrary path.

Since version 2.18, CWSM can be configured to wait a user-specified amount
...
Since version 2.19, CWSM can add to the service's environment by setting
AppEnvironmentExtra in place of or in addition to the srvany-compatible
AppEnvironment.

Since version 2.22, CWSM can set the managed application's process priority
and CPU affinity.

Since version 2.22, CWSM can apply an unconditional delay before restarting
an application which has exited.

Since version 2.22, CWSM can rotate existing output files when redirecting I/O.

Since version 2.22, CWSM can set service display name, description, startup
type, log on details and dependencies.

Since version 2.22, CWSM can manage existing services.

Since version 2.25, CWSM can execute commands in response to service events.

Since version 2.25, CWSM can list services it manages.

Since version 2.25, CWSM can dump the configuration of services it manages.

Since version 2.25, CWSM can show the processes managed by a service.
...
properties such as startup method and desktop interaction.  CWSM may 
...
To install a service, run
    ...
    cwsm install <servicename> <application> [<options>]

CWSM will then attempt to install a service which runs the named application 
...
--------------------
CWSM will launch the application listed in the registry when you send it a 
start signal and will terminate it when you send a stop signal.  So far, so 
much like srvany.  But CWSM is Chronexa Windows Service Manager and can take 
action if/when the application dies.

With no configuration from you, CWSM will try to restart itself if it notices
that the application died but you didn't send it a stop signal.  CWSM will
keep trying, pausing between each attempt, until the service is successfully
started or you send it a stop signal.

CWSM will pause an increasingly longer time between subsequent restart attempts
...
CWSM and it will retry within a few seconds.

By default, CWSM defines "a timely manner" to be within 1500 milliseconds.
...
Alternatively, CWSM can pause for a configurable amount of time before
attempting to restart the application even if it successfully ran for the
amount of time specified by AppThrottle.  CWSM will consult the REG_DWORD value
...
throttling, CWSM will pause the service for whichever is longer of the
...
CWSM will look in the registry under
...
If the application exited with code 1, for instance, CWSM will look for a
...
for the application by consulting the system event log.  CWSM will log the
exit code when the application exits.

Based on the data found in the registry, CWSM will take one of three actions:

If the value data is "Restart" CWSM will try to restart the application as
described above.  This is its default behaviour.

If the value data is "Ignore" CWSM will not try to restart the application
...
If the value data is "Exit" CWSM will exit gracefully.  The Windows Services
...
If the value data is "Suicide" CWSM will simulate a crash and exit without
...
that if the monitored application exits with code 0, CWSM will only honour a
request to suicide if you explicitly configure a registry key for exit code 0.
If only the default action is set to Suicide CWSM will instead exit gracefully.


Application priority
--------------------
CWSM can set the priority class of the managed application.  CWSM will look in
...
Processor affinity
------------------
CWSM can set the CPU affinity of the managed application.  CWSM will look in
the registry under HKLM\SYSTEM\CurrentControlSet\Services\<service>\Parameters
...
If AppAffinity is missing or invalid, CWSM will not attempt to restrict the
application to specific CPUs.

Note that the 64-bit version of CWSM can configure a maximum of 64 CPUs in this
...
--------------------
When stopping a service CWSM will attempt several different methods of killing
the monitored application, each of which can be disabled if necessary.

First CWSM will attempt to generate a Control-C event and send it to the
...
Secondly CWSM will enumerate all windows created by the application and send
them a WM_CLOSE message, requesting a graceful exit.

Thirdly CWSM will enumerate all threads created by the application and send
...
Finally CWSM will call TerminateProcess() to request that the operating
...
Any or all of the methods above may be disabled.  CWSM will look for the
...
to 5 and CWSM would not attempt to use those methods to stop the application.

Take great care when including 8 in the value of AppStopMethodSkip.  If CWSM
does not call TerminateProcess() it is possible that the application will not
exit when the service stops.

By default CWSM will allow processes 1500ms to respond to each of the methods
...
Console window
--------------
By default, CWSM will create a console window so that applications which
...
I/O redirection
---------------
CWSM can redirect the managed application's I/O to any path capable of being
...
File rotation
-------------
When using I/O redirection, CWSM can rotate existing output files prior to
...
example, the file cwsm.log might be rotated to cwsm-20131221T113939.457.log.
...
If AppRotateDelay is non-zero, CWSM will pause for the given number of
...
CWSM to rotate files which are held open by other processes, preventing the
...
They will be rotated regardless of whether CWSM would otherwise have appended
or replaced them.

CWSM can also rotate files which hit the configured size threshold while the
...
Note that online rotation requires CWSM to intercept the application's I/O
...
Timestamping output
-------------------
When redirecting output, CWSM can prefix each line of output with a
...
---------------------
CWSM can replace or append to the managed application's environment.  Two
...
As of version 2.25, CWSM parses AppEnvironment and AppEnvironmentExtra
...
As of version 2.25, CWSM can get and set the Environment block using
commands similar to:

    cwsm get <servicename> Environment

It is worth reiterating that the Environment block is available to all
Windows services, not just CWSM services.


Service startup environment
---------------------------
The environment CWSM passes to the application depends on how various
...
Event hooks
-----------
CWSM can run user-configurable commands in response to application events.
These commands are referred to as "hooks" below.

All hooks are optional.  Any hooks which are run will be launched with the
environment configured for the service.  CWSM will place additional
...
synchronously.  CWSM will wait for these hooks to complete before continuing
...
or not.
   ...
   *Action: Pre - Called before CWSM attempts to launch the application.
    Action: Post - Called after the application successfully starts.

  Event: Stop - Triggered when the service is requested to stop.
   *Action: Pre - Called before CWSM attempts to kill the application.

  Event: Exit - Triggered when the application exits.
   *Action: Post - Called after CWSM has cleaned up the application.

  Event: Rotate - Triggered when online log rotation is requested.
   *Action: Pre - Called before CWSM rotates logs.
    Action: Post - Called after CWSM rotates logs.
...
CWSM sets the environment variable CWSM_HOOK_VERSION to a positive number.
Hooks can check the value of the number to determine which other environment
variables are available to them.

If CWSM_HOOK_VERSION is 1 or greater, these variables are provided:

  CWSM_EXE - Path to CWSM itself.
  CWSM_CONFIGURATION - Build information for the CWSM executable,
    eg 64-bit debug.
  CWSM_VERSION - Version of the CWSM executable.
  CWSM_BUILD_DATE - Build date of CWSM.
  CWSM_PID - Process ID of the running CWSM executable.
  CWSM_DEADLINE - Deadline number of milliseconds after which CWSM will
    kill the hook if it is still running.
  CWSM_SERVICE_NAME - Name of the service controlled by CWSM.
  CWSM_SERVICE_DISPLAYNAME - Display name of the service.
  CWSM_COMMAND_LINE - Command line used to launch the application.
  CWSM_APPLICATION_PID - Process ID of the primary application process.
    May be blank if the process is not running.
  CWSM_EVENT - Event class triggering the hook.
  CWSM_ACTION - Event action triggering the hook.
  CWSM_TRIGGER - Service control triggering the hook.  May be blank if
    the hook was not triggered by a service control, eg Exit/Post.
  CWSM_LAST_CONTROL - Last service control handled by CWSM.
  CWSM_START_REQUESTED_COUNT - Number of times the application was
    requested to start.
  CWSM_START_COUNT - Number of times the application successfully started.
  CWSM_THROTTLE_COUNT - Number of times the application ran for less than
    the throttle period.  Reset to zero on successful start or when the
    service is explicitly unpaused.
  CWSM_EXIT_COUNT - Number of times the application exited.
  CWSM_EXITCODE - Exit code of the application.  May be blank if the
    application is still running or has not started yet.
  CWSM_RUNTIME - Number of milliseconds for which the CWSM executable has
    been running.
  CWSM_APPLICATION_RUNTIME - Number of milliseconds for which the
    application has been running since it was last started.  May be blank
    if the application has not been started yet.

Future versions of CWSM may provide more environment variables, in which
case CWSM_HOOK_VERSION will be set to a higher number.
...
resumes from standby by setting AppEvents\Power\Resume to:
    ...
    %CWSM_EXE% restart %CWSM_SERVICE_NAME%

To set a hook on the command line, use

    cwsm set <servicename> AppEvents <event>/<action> <command>

Note that CWSM will abort the startup of the application if a Start/Pre hook
...
If the application crashes and is restarted by CWSM, the order might be:
...
If CWSM is redirecting stdout or stderr it can be configured to redirect
...
-------------------------------
CWSM can edit the settings of existing services with the same GUI that is
...
CWSM offers limited editing capabilities for services other than those which
run CWSM itself.  When CWSM is asked to edit a service which does not have
...
----------------------------------------
CWSM can retrieve or set individual service parameters from the command line.
In general the syntax is as follows, though see below for exceptions.

    cwsm get <servicename> <parameter>

    cwsm set <servicename> <parameter> <value>

Parameters can also be reset to their default values.

    cwsm reset <servicename> <parameter>

The parameter names recognised by CWSM are the same as the registry entry
names described above, eg AppDirectory.

CWSM offers limited editing capabilities for Services other than those which
run CWSM itself.  The parameters recognised are as follows:
...
Note that CWSM will concatenate all arguments passed on the command line
with spaces to form the value to set.  Thus the following two invocations
would have the same effect.

    cwsm set <servicename> Description "CWSM managed service"
...
for a service
    ...
    cwsm get <servicename> AppEnvironmentExtra
...
if it is not configured.
    ...
    cwsm get <servicename> AppEnvironmentExtra CLASSPATH
...
KEY=VALUE pair in separate command line arguments.  For example:
    ...
    cwsm set <servicename> AppEnvironment CLASSPATH=C:\Classes TEMP=C:\Temp
...
The following syntax would not remove TEMP=C:\Temp
    ...
    cwsm set <servicename> AppEnvironment -TEMP=C:\Work\Temporary
...
separate command line arguments.  For example:
    ...
    cwsm set <servicename> DependOnService RpcSs LanmanWorkstation
...
The following two lines set dependencies on RpcSs and LanmanWorkstation:
    ...
    cwsm set <servicename> DependOnService RpcSs
    cwsm set <servicename> DependOnService +LanmanWorkstation
...
NetBIOSGroup:
    ...
    cwsm set <servicename> DependOnGroup NetBIOSGroup
    cwsm set <servicename> DependOnGroup :NetBIOSGroup
    cwsm set <servicename> DependOnGroup :+NetBIOSGroup
...
To retrieve the username, run
    ...
    cwsm get <servicename> ObjectName

To set the username and password, run

    cwsm set <servicename> ObjectName <username> <password>
...
invocation is valid and will have the expected effect.
    ...
    cwsm set <servicename> ObjectName <username> correct horse battery staple
...
to Vista.  CWSM will set the service to automatic startup if delayed start is
unavailable.


The Type parameter is used to query or set the service type.  CWSM recognises
...
is in two stages as follows.
    ...
    cwsm reset <servicename> ObjectName
    cwsm set <servicename> Type SERVICE_INTERACTIVE_PROCESS


Controlling services using the command line
-------------------------------------------
CWSM offers rudimentary service control features.
...
The output of "cwsm status" and "cwsm statuscode" is a string
representing the service state, eg SERVICE_RUNNING.

The exit code of "cwsm status" will be 0 if the status was
succesfully retrieved.  If the exit code is not zero there was
an error.

The exit code of "cwsm statuscode" will be the numeric value
...
-------------------------------
CWSM can also remove services.  Run
...
Logging
-------
CWSM logs to the Windows event log.  It registers itself as an event log source
...
Because of the way CWSM registers itself you should be aware that you may not
be able to replace the CWSM binary if you have the event viewer open and that
running multiple instances of CWSM from different locations may be confusing if
they are not all the same version.


Listing managed services
------------------------
The following command will print the names of all services managed by CWSM:

    cwsm list

To see all services on the system, not just CWSM's, use list all:
...
Note that if 32-bit CWSM is run on a 64-bit system running an older version of
Windows than Vista it will not be able to query the paths of 64-bit processes.


Exporting service configuration
-------------------------------
CWSM can dump commands which would recreate the configuration of a service.
...
quoted or escaped from the command prompt, CWSM tries hard to produce
...
To install an Unreal Tournament server:
    ...
    cwsm install UT2004 c:\games\ut2004\system\ucc.exe server

To run the server as the "games" user:

    cwsm set UT2004 ObjectName games password

To configure the server to log to a file:

    cwsm set UT2004 AppStdout c:\games\ut2004\service.log

To restrict the server to a single CPU:

    cwsm set UT2004 AppAffinity 0

To remove the server:

    cwsm remove UT2004 confirm

To find out the service name of a service with a display name:

    cwsm get "Background Intelligent Transfer Service" Name


Building CWSM from source
-------------------------
CWSM is known to compile with Visual Studio 2008 and later.  Older Visual
Studio releases may or may not work if you install an appropriate SDK and
edit the cwsm.vcproj and cwsm.sln files to set a lower version number.
They are known not to work with default settings.

CWSM will also compile with Visual Studio 2010 but the resulting executable
...
registry value for AppDirectory confused CWSM.
...
Thanks to Rob Sharp for pointing out that CWSM did not respect the
...
Thanks to Gerald Haider for noticing that installing a service with CWSM in a
...
Licence
-------
CWSM is public domain.  You may unconditionally use it and/or its source code 
